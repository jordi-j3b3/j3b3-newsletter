...
**Desfase del CDMGE (regla sin excepción):** cuando la cifra protagonista del
bloque 1 procede del CDMGE (`pulso_diario.csv`, indicador del INE sobre grandes
cadenas de distribución), el bloque 1 debe incluir siempre dos elementos
explícitos:

1. La fecha exacta del dato más reciente disponible (campo
   `pulso_diario.ultima_fecha` en `_meta.json`).
2. La nota de desfase: *"Dato más reciente disponible a [fecha]. El indicador
   del INE sobre grandes cadenas se publica con un desfase habitual de 30 días."*

La nota puede ir en el campo **Fuente:** o integrada en el cuerpo, pero no puede
omitirse. El lector necesita saber cuándo es el dato, no solo qué vale.
